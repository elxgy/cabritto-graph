...
def build_binary_tree(adj_list, root_value, visit_counts=None):
    if visit_counts is None:
        visit_counts = {}

    # Atualiza a contagem para esse nó
    visit_counts[root_value] = visit_counts.get(root_value, 0) + 1

    # Se já foi visitado mais de uma vez, retornamos o nó sem aprofundar para evitar ciclo infinito.
    if visit_counts[root_value] > 1:
        return BinaryTreeNode(root_value)

    # Se o nó não está na lista de adjacência, não há filhos.
    if root_value not in adj_list:
        return BinaryTreeNode(root_value)
    ...
    if len(children) > 0:
        if children[0] is not None:
            root.left = build_binary_tree(adj_list, children[0], new_visit_counts)
    if len(children) > 1:
        if children[1] is not None:
            root.right = build_binary_tree(adj_list, children[1], new_visit_counts)
    return root
...
def build_nary_tree(adj_list, root_value, visit_counts=None):
    if visit_counts is None:
        visit_counts = {}
    ...
    for child in children:
        if child is not None:
            child_node = build_nary_tree(adj_list, child, new_visit_counts)
    ...
    try:
        ...
        if binary_tree_check(working_adj_list):
            ...
            if root_value is not None:
                ...
                # Use a versão atualizada para lidar com ciclos
